"""Run Step 25 compact price-action-first feature-family audit."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("artifacts/research/compact-feature-audit-v1"),
    )
    ...
    payload = run_compact_feature_audit(
        context=context,
        minute_candles=candles[Timeframe.M1],
    ...
    experiment_id = str(
        uuid5(
            NAMESPACE_URL,
            f"step25:{args.dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}",
        )
    )
