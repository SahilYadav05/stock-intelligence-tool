"""Run Step 23 conditional opportunity/direction research."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset-id",
        required=True,
        help="Immutable PASS dataset ID; required to prevent accidental dataset drift",
    )
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("artifacts/research/conditional-direction-v1"),
    )
    args = parser.parse_args()
    _require_live_signal_kill_switch(args.env_file)
    repository = SQLiteHistoricalRepository(args.database)
    dataset_id = args.dataset_id
    quality = repository.load_dataset_quality_status(dataset_id=dataset_id)
    ...
    print("Step 23 conditional opportunity/direction research")
    print(f"- pinned NIFTY dataset: {dataset_id}")
    print(f"- stored dataset quality: {quality.value}")
    print(f"- context bundle: {context_path}")
    print("- opportunity and direction are fitted as separate causal heads")
    print("- final two chronological folds remain diagnostic only")
    ...
    payload = run_conditional_direction_research(
        context=context,
        minute_candles=candles[Timeframe.M1],
    ...
    experiment_id = str(
        uuid5(
            NAMESPACE_URL,
            f"step23:{dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}",
        )
    )
    ...
    print("RESULT: STEP 23 CONDITIONAL DIRECTION RESEARCH COMPLETED")
