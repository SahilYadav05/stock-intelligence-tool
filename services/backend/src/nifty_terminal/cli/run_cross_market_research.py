"""Run Step 18C NIFTY-only versus cross-market controlled research."""
...
from nifty_terminal.cli.run_real_data_research import DEFAULT_CALENDAR, _require_live_signal_kill_switch, _write_immutable_json
...
from nifty_terminal.research.step18c import RESEARCH_IDENTITY, run_cross_market_research
...
def main() -> None:
    ...
    parser.add_argument("--report-dir", type=Path, default=Path("artifacts/research/cross-market-v1"))
    args = parser.parse_args()
    _require_live_signal_kill_switch(args.env_file)
    repository = SQLiteHistoricalRepository(args.database)
    dataset_id = args.dataset_id or repository.latest_pass_dataset_id(instrument_id="NIFTY50_SPOT")
    if dataset_id is None:
        raise SystemExit("No immutable PASS NIFTY50_SPOT dataset is available")
    candles = {timeframe: repository.load_latest_candles(
        dataset_id=dataset_id, instrument_id="NIFTY50_SPOT", timeframe=timeframe
    ...
    print("Step 18C controlled cross-market model research")
    print(f"- NIFTY dataset: {dataset_id}")
    print(f"- context bundle: {context_path}")
    print("- context: finalized Bank Nifty and India VIX only")
    print("- experiment: same samples, targets, folds, costs and candidates")
    print("- comparison: NIFTY-only versus context-aware")
    print("- no missing context is imputed; exact finalized 5m joins only")
    print("- no news, breadth, futures/OI or chart images are fabricated")
    print("- runtime may be 40-120 minutes on a local CPU")
    ...
    context = build_context_feature_matrix(
        dataset=dataset, primary_candles=candles[Timeframe.M5], bundle=bundle
    )
    if context.dataset.eligible_samples < 15_000:
        raise SystemExit("Context alignment left fewer than 15,000 eligible samples")
    payload = run_cross_market_research(
        context=context,
        minute_candles=candles[Timeframe.M1],
        primary_candles=candles[Timeframe.M5],
    ...
    experiment_id = str(uuid5(NAMESPACE_URL, f"step18c:{dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}"))
    ...
    printable = {
        ...
        "context_feature_build": payload["context_feature_build"],
        "controlled_comparison": payload["controlled_comparison"],
    ...
    print("RESULT: STEP 18C CROSS-MARKET RESEARCH COMPLETED")
    print("No model or signal was released; a failed gate remains a valid result.")


def _latest_context_bundle(directory: Path) -> Path:
    candidates = tuple(directory.glob("context-*.json.gz"))
    if not candidates:
        raise SystemExit(
            "No context bundle found. Run acquire:context:angelone:windows first."
        )
