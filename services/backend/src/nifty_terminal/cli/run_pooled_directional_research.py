"""Run Step 20 pooled directional, coverage-constrained research."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("artifacts/research/pooled-directional-v1"),
    )
    ...
    print("Step 20 pooled directional coverage research")
    print(f"- NIFTY dataset: {dataset_id}")
    print(f"- context bundle: {context_path}")
    print("- pooled LONG/SHORT meta-labels with explicit direction interactions")
    print("- seven purged chronological folds; final two are diagnostic only")
    print("- trade cadence, win rate, expectancy and baseline uplift are hard gates")
    ...
    payload = run_pooled_directional_research(
        context=context,
        minute_candles=candles[Timeframe.M1],
    ...
    experiment_id = str(
        uuid5(NAMESPACE_URL, f"step20:{dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}")
    )
    ...
    print("RESULT: STEP 20 POOLED DIRECTIONAL RESEARCH COMPLETED")
