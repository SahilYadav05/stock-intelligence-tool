"""Run Step 19 causal price-action, baseline-controlled research."""
...
from nifty_terminal.research.step19 import (
    RESEARCH_IDENTITY,
    STEP19_VERSION,
...
def main() -> None:
    ...
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("artifacts/research/price-action-v1"),
    )
    ...
    context_path = args.context_bundle or _latest_context_bundle(Path("artifacts/context/angelone"))
    bundle = read_bundle(context_path)
    print("Step 19 causal price-action baseline-controlled research")
    print(f"- NIFTY dataset: {dataset_id}")
    print(f"- context bundle: {context_path}")
    print("- new information: confirmed swings, structure breaks, sweeps and compression")
    print("- locked Step 18F gates and execution assumptions are unchanged")
    print("- the historical period remains previously seen and cannot approve release")
    ...
    payload = run_price_action_research(
        context=context,
        minute_candles=candles[Timeframe.M1],
    ...
    experiment_id = str(
        uuid5(NAMESPACE_URL, f"step19:{dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}")
    )
    ...
    printable = {
        ...
        "step19_version": STEP19_VERSION,
        "price_action_features": payload["price_action_features"],
    ...
    print("RESULT: STEP 19 PRICE-ACTION RESEARCH COMPLETED")
