"""Run Step 18E leakage-safe ranked utility cohort research."""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=Path("artifacts/research/ranked-utility-v1"),
    )
    ...
    print("Step 18E leakage-safe ranked utility cohort research")
    print(f"- NIFTY dataset: {dataset_id}")
    print(f"- context bundle: {context_path}")
    print("- objective: rank realized LONG/SHORT R under the locked execution rules")
    print("- architecture selection: " + ", ".join(ARCHITECTURES))
    print("- model candidates: " + ", ".join(MODEL_CANDIDATES))
    print("- chronology: model selection, score reference, policy selection, later diagnostic")
    print("- score percentiles use prior scores only; no future distribution is used")
    print("- uncertainty: session-block bootstrap over the selected trade cohort")
    print("- replay: next 1m open, stop first, 0.5 point one-way slippage, no overlap")
    print("- policy must independently support BUY and SELL with positive lower-bound expectancy")
    print("- no gate or threshold is forced to pass")
    print("- existing shadow runtime will not be modified")
    print("- official signals and automatic trading remain disabled")
    print("- local CPU runtime may be 30-90 minutes")
    ...
    payload = run_ranked_utility_research(
        context=context,
        minute_candles=candles[Timeframe.M1],
    ...
    experiment_id = str(uuid5(
        NAMESPACE_URL,
        f"step18e:{dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}",
    ))
    ...
    printable = {
        ...
        "feature_architectures": payload["feature_architectures"],
        ...
        "methodological_correction": payload["methodological_correction"],
    ...
    print("RESULT: STEP 18E RANKED UTILITY RESEARCH COMPLETED")
    print("No model or signal was released; failed gates remain valid evidence.")
