"""Run Step 18D selective utility research on canonical cross-market data."""
...
from nifty_terminal.research.step18d import (
    CANDIDATES,
    RESEARCH_IDENTITY,
    run_selective_utility_research,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--report-dir", type=Path, default=Path("artifacts/research/selective-utility-v1")
    )
    ...
    print("Step 18D selective utility and conformal downside research")
    print(f"- NIFTY dataset: {dataset_id}")
    print(f"- context bundle: {context_path}")
    print("- objective: realized LONG/SHORT R, not arbitrary direction confidence")
    print("- uncertainty: out-of-sample conformal 20th-percentile lower estimate")
    print("- features: finalized NIFTY, Bank Nifty and India VIX only")
    print(f"- candidates: {', '.join(CANDIDATES)}")
    print("- chronology: model selection, uncertainty fit, policy selection, later diagnostic")
    print("- replay: next 1m open, stop first, 0.5 point one-way slippage, no overlap")
    print("- NIFTY-only ablation must be beaten by the context model")
    print("- no thresholds are forced to pass")
    ...
    payload = run_selective_utility_research(
        context=context,
        minute_candles=candles[Timeframe.M1],
    ...
    experiment_id = str(uuid5(
        NAMESPACE_URL,
        f"step18d:{dataset_id}:{RESEARCH_IDENTITY}:{created_at.isoformat()}",
    ))
    ...
    print("RESULT: STEP 18D SELECTIVE UTILITY RESEARCH COMPLETED")
